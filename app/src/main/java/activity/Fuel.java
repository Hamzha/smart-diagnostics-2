...
public class Fuel extends BaseClass {
    int timer = 15000;
    ...
    private void init() {
        ...
        Toolbar(context, "Fuel Graph/" + device.getName());
        ...
        fetchRouteFromServer(startDate[0], endDate[0]);
    }
    ...
    private void setDataSet(Position[] routes) {
        ...
        for (Position route : routes) {
            if (route.getPositionAttributes().getMotion() != null && !route.getPositionAttributes().getMotion()) {
                if (device.getAttributes().getFuelLow() != null && device.getAttributes().getFuelMiddle() != null && device.getAttributes().getFuelHigh() != null) {
                    float fuel = (float) (route.getPositionAttributes().getFuel() * route.getPositionAttributes().getFuel() * Double.parseDouble(device.getAttributes().getFuelHigh()));
                    fuel = (float) (fuel + (route.getPositionAttributes().getFuel() * Double.parseDouble(device.getAttributes().getFuelMiddle())));
                    fuel = (float) (fuel + Double.parseDouble(device.getAttributes().getFuelLow()));

                    Common.logd(TAG, String.valueOf(fuel));


                    if (highest_value < fuel) {
                        highest_value = fuel;
                    }
                    dataSet.add(new Entry(++count, fuel));
                } else
                    dataSet.add(new Entry(++count, (float) route.getPositionAttributes().getFuel()));
            }
        }

        LineDataSet set1 = new LineDataSet(dataSet, "Fuel");
    ...
    private void initLineChart() {
        ...
        {   // // Y-Axis Style // //
            ...
            Common.logd(TAG, String.valueOf(highest_value));
